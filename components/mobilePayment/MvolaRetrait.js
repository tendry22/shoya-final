...
import Mvola from "../../assets/m_vola.png";

const MvolaRetrait = () => {
  const [montant, setMontant] = useState("");
  const [numeroSelectionne, setNumeroSelectionne] = useState(null); // État pour suivre le numéro sélectionné
  ...
  const handleNumeroSelection = (numero) => {
    setNumeroSelectionne(numero);
  };

  const handleSubmit = () => {
    if (montant === "" || montant < 25000) {
      ToastAndroid.show("Veuillez vérifier les champs", ToastAndroid.SHORT);
    } else if (numeroSelectionne === null) {
      ToastAndroid.show("Veuillez choisir un numéro", ToastAndroid.SHORT);
    } else {
      Alert.alert(
  ...
  return (
    ...
    >
      ...
      <View style={styles.formCard}>
        <Text style={styles.texte}>Faire un retrait</Text>
        <View style={styles.container}>
          <View style={styles.inputContainer}>
            ...
            <Text style={styles.label}>Choisir le numéro</Text>
            <View style={styles.numContainer}>
              <TouchableOpacity
                onPress={() => handleNumeroSelection("+261328562147")}
              >
                <View
                  style={[
                    styles.numLIst,
                    numeroSelectionne === "+261328562147" && styles.selectedNum,
                  ]}
                >
                  <Image
                    source={Orange}
                  ...
                  />
                  <Text style={styles.numTxt}>+261328562147</Text>
                </View>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => handleNumeroSelection("+261342563214")}
              >
                <View
                  style={[
                    styles.numLIst,
                    numeroSelectionne === "+261342563214" && styles.selectedNum,
                  ]}
                >
                  <Image
                    source={Mvola}
                    style={{ width: 30, height: 30, borderRadius: 15 }}
                  />
                  <Text style={styles.numTxt}>+261342563214</Text>
                </View>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => handleNumeroSelection("+261388562146")}
              >
                <View
                  style={[
                    styles.numLIst,
                    numeroSelectionne === "+261388562146" && styles.selectedNum,
                  ]}
                >
                  <Image
                    source={Mvola}
                    style={{ width: 30, height: 30, borderRadius: 15 }}
                  />
                  <Text style={styles.numTxt}>+261388562146</Text>
                </View>
              </TouchableOpacity>
            </View>
