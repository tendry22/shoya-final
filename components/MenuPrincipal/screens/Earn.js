    ...
    try {
      ...
      if (jwt_token) {
        ...
        if (refferenceData && refferenceData.length > 0) {
          const userRefference = refferenceData.find(
          ...
          if (userRefference) {
            ...
            console.error(
              "Aucun code de parrainage trouvé pour cet utilisateur."
            );
            ToastAndroid.show(
        ...
        ToastAndroid.show(
          "Erreur lors de la récupération du code de parrainage dans le jwt",
          ToastAndroid.LONG
        );
      }
    } catch (error) {
          ...
          <View style={styles.modalView}>
            <View style={styles.modalContent}>
              ...
              <View style={{ alignItems: "center" }}>
                {/* Utilisation de react-native-qrcode-styled */}
                <QRCodeStyled
                  data={refference.toString()}
                  pieceSize={10}
                  color={"#B6EA5C"}
                />
              </View>
...
const styles = StyleSheet.create({
  ...
  modalContent: {
    backgroundColor: "rgba(24, 21, 38, 1)",
    borderRadius: 20,
    padding: 35,
    elevation: 5,
    width: "75%",
