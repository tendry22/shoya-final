...
const ConnectWallet = () => {
  ...
  const [showError, setShowError] = useState(false); // Nouvel état pour contrôler la visibilité de la vue rouge
  ...
  const handleEmailSubmit = () => {
    ...
    if (isValidEmail(email) && !isSpecialEmail) {
      ...
      if (is) {
        ...
        ToastAndroid.show("Vous n'etes pas administrateur", ToastAndroid.LONG);
      }
    } else {
  ...
  return (
    <ImageBackground source={bgImage} style={styles.container}>
      ...
      >
        ...
        {/* Afficher la vue rouge si l'e-mail n'est pas trouvé */}
        {showError && (
          <View style={styles.errorView}>
            <Ionicons name="alert-circle" color={"red"} size={15} />
            <Text style={styles.errorText}>
              Cette email n'existe pas, veuiller vérifier
            </Text>
          </View>
        )}
