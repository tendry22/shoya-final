...
const DepotTether = () => {
  ...
  useEffect(() => {
    const fetchMin = async () => {
      try {
        ...
        const mga = response.data[0].usdt;
        setMin(mga);
      } catch (error) {
  ...
  const pickImage = async () => {
    ...
    if (result && result.assets[0].uri) {
      try {
        ...
        setAddress(dataNeeded);
        toggleQrModal(false);
      } catch (error) {
        // if there this no QR code
        ToastAndroid.show("QR Code non trouvé sur l'image", ToastAndroid.LONG);
      ...
      if (jwt_token) {
        try {
          ...
          if (Array.isArray(adresses) && adresses.length > 0) {
            ...
            const filteredAdresses = adresses.filter(
              (adresse) => adresse.iduser === userId
            );
    ...
    try {
      ...
      if (jwt_token) {
        ...
        const response = await Axios.post(apiUrl, {
          iduser: userId,
          adresse: newAdresseUSDT,
          libelle: newLibelleUSDT,
          reseau: newReseauUSDT,
        });
  ...
  const handleSelect = async (value) => {
    ...
    const adresse = await Axios.post(`${BASE_URL}/binance/network-adress`, {
      network: value,
    });
    ...
    <View style={styles.container}>
      <View style={styles.inputContainer}>
        ...
        <View style={styles.vw}>
          ...
          <TouchableOpacity style={{ marginLeft: 10 }} onPress={toggleQrModal}>
            <Icon
              style={{ paddingBottom: 15 }}
              name="qrcode"
              size={18}
                          ...
                          <ScrollView>
                            {networkList.map((item, index) => (
                              <TouchableOpacity
                                key={index}
                                onPress={() =>
                                  handleSelectNewReseau(item.network)
                                } // Utilisez handleSelectNewReseau ici
                                style={[
                                  styles.optionCard,
                                  selectedValueReseau === item.network &&
                                    styles.selectedOption, // Assurez-vous de comparer avec selectedValueReseau
                                ]}
                              >
      ...
      <View style={styles.inputContainer}>
        ...
        <View style={styles.minMaxContainer}>
          ...
          <View>
            {maxMga / cours > min ? (
              <>
                <Text style={styles.minMaxValueText}>{min} USD</Text>
                <Text style={styles.minMaxValueText}>
                  {(maxMga / cours).toFixed(2)} USD
                </Text>
              </>
            ) : (
              <Text style={{ ...styles.minMaxValueText, color: "red" }}>
                Solde insuffisant
              </Text>
            )}
          </View>
        ...
        >
          ...
          >
            <Camera
              ...
              onBarCodeScanned={(...args) => {
                ...
                setAddress(data);
                setQrModalVisible(false); // Fermer le modal une fois que la valeur est chargée
              }}
            />
          </View>
          <TouchableOpacity onPress={pickImage}>
            ...
            >
              ...
              <Text style={{ fontFamily: "OnestBold", marginLeft: "4%" }}>
